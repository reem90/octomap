...
void print_query_info(point3d query, LabelOcTreeNode* node) {
  if (node != NULL) {
...
int main(int argc, char** argv) {
  ...
  for (int x=-30; x<30; x++) {
    for (int y=-30; y<30; y++) {
      for (int z=-30; z<30; z++) {
        ...
        LabelOcTreeNode* n = tree.updateNode(endpoint, false);
        n->setLabel(2); // set label to yellow ********
      }
    }
  }

  // set inner node labels
  ...
  std::string filename ("simple_label_tree.ot");
  std::cout << "Writing label tree to " << filename << std::endl;
  // write label tree
  EXPECT_TRUE(tree.write(filename));


  // read tree file
  cout << "Reading label tree from "<< filename <<"\n";
  ...
  LabelOcTree* read_label_tree = dynamic_cast<LabelOcTree*>(read_tree);
  EXPECT_TRUE(read_label_tree);

 // EXPECT_TRUE(tree == *read_label_tree);
...

  {
    ...
    point3d query (0., 0., 0.);
    LabelOcTreeNode* result = tree.search (query);
    LabelOcTreeNode* result2 = read_label_tree->search (query);
    std::cout << "READ: ";
    ...
    result2 = read_label_tree->search (query);
    print_query_info(query, result);
    std::cout << "READ: ";
    print_query_info(query, result);
    std::cout << "WRITE: ";
...
//    LabelOcTreeNode* newNode = tree.updateNode(newCoord, true);
//    //newNode->setLabel(3); //****
...
//    LabelOcTreeNode* parentNode = tree.search(newCoord, tree.getTreeDepth() -1); //****
...
//    EXPECT_TRUE(tree.write("simple_label_tree_ed.ot"));
